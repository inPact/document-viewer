...
export default class BillService {
    ...
    resolveTotals(variables, collections) {
        ...
        if (variables.TOTAL_TIPS_ON_PAYMENTS !== undefined && variables.TOTAL_TIPS_ON_PAYMENTS > 0 && isServiceCharge == true) {
            ...
            //TRANS_TYPE is only set for credit-card/gift-card transactions, so key off
            //PAYMENT_TYPE, which is set for every refund tender - credit card, room charge,
            //house account, cash and so on (TAB-48986)
            const PAYMENT_TYPE = _.get(collections, 'PAYMENT_LIST[0].PAYMENT_TYPE');
